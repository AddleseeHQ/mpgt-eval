...
def goal_mask(data, goal_mask_ratio):
    new_data = []
    print(data)
    for i in range(len(data)):
        s = data[i].split()
        cur = []
        if random.random() < goal_mask_ratio:
            print(s)
            idx = find_str(s, '@')
            print(idx)
            print(s[0])
...
def add_noise(data, window_size):
    ...
    # then goal mask if goals in window
    if '@' in window_data:
        noise_data = goal_mask(window_data, args.goal_mask_ratio)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--goal_mask_ratio', default=0.5,
        help='Ratio of goals in the window that will be masked', type=float)
